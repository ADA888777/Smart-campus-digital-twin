 ...
 * down with it.
 * ------------------------------------------------------------------ */
function recomputeStatus(failedDevices = new Set()) {
  const db = store.db;
  const adj = new Map();
...
function runScenario(kind) {
  ...
  if (kind === 'restore') {
    ...
    db.devices.forEach((d) => { d.status = 'online'; d.risk = 'low'; d.access = 'allow'; });
  ...
  if (kind === 'device-failure') {
    ...
    runWorkflow({
      ...
      action: `Marked the switch and ${Math.max(analysis.affectedCount - 1, 0)} endpoints offline, opened ${alert.id}`,
  ...
  if (kind === 'high-traffic') {
    ...
    runWorkflow({
      ...
      action: `Set four devices to warning and opened ${alert.id}`,
  ...
  if (kind === 'unauthorized-device') {
    ...
    runWorkflow({
      ...
      action: `Access set to DENY, risk set to HIGH, ${alert.id} opened`,
